...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
		...
		if (md5($password) === $row['password']) {
			...
			$_SESSION['lastname'] = $row['lastname']; // Store lastname in session
			$_SESSION['id'] = $row['id']; 
			$_SESSION['address'] = $row['address']; // Store address in session
			$_SESSION['phone_number'] = $row['phone_number']; // Store phone_number in session
			$_SESSION['gender'] = $row['gender']; // Store address in session
